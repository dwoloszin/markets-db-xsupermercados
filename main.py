...
if __name__ == "__main__":
    ...
    if canonical_scrape_market and canonical_scrape_market in _market_to_mode:
        ...
        # 1b. Sync THIS market's rows into app_offers (manager2) right after saving
        #     raw offers — deliberately BEFORE the barcode pipeline so that a slow or
        #     hanging inference step can never block the app-facing table from
        #     refreshing. This is what keeps app_offers current in the daily
        #     per-market GitHub Actions runs.
        try:
            from sync_app_offers import prepare_sync as _prepare_app_offers_sync
            from sync_app_offers import sync_one_market as _sync_one_market
            _ceps = _prepare_app_offers_sync()
            _market_url = system.db._get_market_database_url(canonical_scrape_market)
            _synced = _sync_one_market(_market_url, _ceps)
            print(f"  app_offers: {canonical_scrape_market} -> {_synced:,} rows synced")
        except Exception as _sync_exc:
            print(f"  app_offers sync warning ({canonical_scrape_market}): {_sync_exc}")
