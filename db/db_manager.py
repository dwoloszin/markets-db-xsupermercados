...
class DatabaseManager:
    ...
    def _connect_pg(self, database_url: str):
        ...
        for attempt in range(1, max_retries + 1):
            try:
                # prepare_threshold=None disables psycopg3 auto-preparation, which
                # collides under the Supabase transaction pooler (port 6543) where
                # consecutive statements land on different server backends
                # (DuplicatePreparedStatement "_pg3_0 already exists").
                return psycopg.connect(
                    database_url, connect_timeout=20, prepare_threshold=None
                )
    ...
    def get_catalog_sync_epoch(self, sync_key: str) -> Optional[float]:
        """Shared, cross-repo last-catalog-sync timestamp (epoch seconds).

        Stored in the manager DB so all 16 market repos see the same value —
        a local file can't coordinate across separate GitHub Actions runners.
        Returns None on any error so the caller fails safe (re-syncs)."""
        if not self.use_postgres:
            return None
        try:
            conn = self._connect_pg(self.database_url_manager)
            conn.autocommit = True
            cursor = conn.cursor()
            cursor.execute(
                "CREATE TABLE IF NOT EXISTS catalog_sync_state ("
                "sync_key TEXT PRIMARY KEY, last_synced_at TIMESTAMPTZ NOT NULL)"
            )
            cursor.execute(
                "SELECT EXTRACT(EPOCH FROM last_synced_at) FROM catalog_sync_state WHERE sync_key = %s",
                (sync_key,),
            )
            row = cursor.fetchone()
            conn.close()
    ...
    def mark_catalog_sync(self, sync_key: str) -> None:
        """Record that the shared catalog was just synced (see get_catalog_sync_epoch)."""
        if not self.use_postgres:
            return
        try:
            conn = self._connect_pg(self.database_url_manager)
            ...
            cursor.execute(
                "INSERT INTO catalog_sync_state (sync_key, last_synced_at) "
                "VALUES (%s, NOW()) "
                "ON CONFLICT (sync_key) DO UPDATE SET last_synced_at = NOW()",
                (sync_key,),
            )
            conn.close()
        except Exception:
    ...
    def optimize_database(self) -> dict:
        if self.use_postgres:
            # Only ANALYZE the manager DB (shared catalog / inference tables) to keep
            # query plans fresh. Reclaim is handled by the aggressive per-table
            # autovacuum set on the churny tables, so we no longer VACUUM every DB
            # on every run — that was ~50s of redundant work in each of 16 repos.
            try:
                conn = self._connect_pg(self.database_url_manager)
                conn.autocommit = True
                cursor = conn.cursor()
                ...
                print(f"optimize_database: manager ANALYZE skipped ({exc})")
            return {
                "db_path": "postgres",
                "size_before_bytes": 0,
                "size_after_bytes": 0,
    ...
    @classmethod
    def _clean_catalog_description(cls, value: Any) -> Optional[str]:
        """Strip HTML tags, collapse whitespace, and cap length for storage."""
        if value is None:
            return None
        text = str(value)
        if "<" in text:
        ...
        if not text:
            return None
        return text[: cls.CATALOG_DESCRIPTION_MAX]
    ...
    def upsert_known_barcodes(self, rows: Sequence[Tuple[Any, ...]]) -> int:
        ...
        # Also strip HTML/whitespace from the description and cap it: some markets
        # return full HTML product pages (up to ~370 KB) which bloat the catalog.
        def _pad(row: tuple) -> tuple:
            ...
            lst[5] = self._clean_catalog_description(lst[5])  # description column
            return tuple(lst)
        ...
        if self.use_postgres:
            sql = """
                ...
                ON CONFLICT (barcode) DO UPDATE SET
                    source_market     = COALESCE(product_catalog.source_market,    EXCLUDED.source_market),
                    source_market_id  = COALESCE(product_catalog.source_market_id, EXCLUDED.source_market_id),
                    ...
                    last_updated      = EXCLUDED.last_updated
                -- Skip no-op rewrites: only touch the row (and bump last_updated)
                -- when some field would actually change. This stops the whole
                -- catalog from churning every day, which was bloating the heap.
                WHERE
                    product_catalog.canonical_name        IS DISTINCT FROM COALESCE(EXCLUDED.canonical_name,        product_catalog.canonical_name)
                    OR product_catalog.canonical_brand    IS DISTINCT FROM COALESCE(EXCLUDED.canonical_brand,       product_catalog.canonical_brand)
                    OR product_catalog.canonical_description IS DISTINCT FROM COALESCE(EXCLUDED.canonical_description, product_catalog.canonical_description)
                    OR product_catalog.normalized_name    IS DISTINCT FROM COALESCE(EXCLUDED.normalized_name,       product_catalog.normalized_name)
                    OR product_catalog.normalized_brand   IS DISTINCT FROM COALESCE(EXCLUDED.normalized_brand,      product_catalog.normalized_brand)
                    OR product_catalog.measure_token      IS DISTINCT FROM COALESCE(EXCLUDED.measure_token,         product_catalog.measure_token)
                    OR product_catalog.image_url          IS DISTINCT FROM COALESCE(EXCLUDED.image_url,             product_catalog.image_url)
                    OR product_catalog.market_count       IS DISTINCT FROM GREATEST(EXCLUDED.market_count,          product_catalog.market_count)
                    OR product_catalog.source_market      IS DISTINCT FROM COALESCE(product_catalog.source_market,    EXCLUDED.source_market)
                    OR product_catalog.source_market_id   IS DISTINCT FROM COALESCE(product_catalog.source_market_id, EXCLUDED.source_market_id)
            """
            ...
            def _upsert_batch_with_retry(batch_rows: Sequence[Tuple[Any, ...]]) -> int:
                ...
                for attempt in range(1, max_attempts + 1):
                    ...
                    try:
                        conn = self._get_pg_for_common_table("product_catalog")
                        cursor = conn.cursor()
                        # Bound how long we'll wait for the shared advisory lock. If a
                        # concurrent runner is holding it, fail fast (55P03) and let the
                        # retry loop handle it, instead of blocking up to the 90-min CI
                        # timeout. SET LOCAL is transaction-scoped → safe under the
                        # transaction pooler (startup 'options' are ignored there).
