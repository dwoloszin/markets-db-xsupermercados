...
class BarcodeAIMatcher:
    ...
    def _get_last_sync_epoch(self, sync_key: str) -> Optional[float]:
        # Prefer the shared DB timestamp so the 16 market repos coordinate the
        # once-a-day catalog sync (a local file can't — each runner is isolated).
        if getattr(self.db, "use_postgres", False):
            return self.db.get_catalog_sync_epoch(sync_key)
    ...
    def _mark_sync_now(self, sync_key: str) -> None:
        if getattr(self.db, "use_postgres", False):
            self.db.mark_catalog_sync(sync_key)
            return
        payload = self._read_sync_state()
